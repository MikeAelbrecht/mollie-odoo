...
class PaymentTransaction(models.Model):
    ...
    def _process_notification_data(self, data):
        ...
        if payment_status == "paid":
            ...
            self._set_canceled(state_message="Mollie: " + _("Mollie: canceled due to status: %s", payment_status))
        elif payment_status == "open":
            self._set_error(state_message="Mollie: " + _("A payment was started, but not finished: %s", payment_status))
        else:
            _logger.info("Received data with invalid payment status: %s", payment_status)
            self._set_error(
                state_message="Mollie: " + _("Received data with invalid payment status: %s", payment_status)
            )
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if payment_data.get("_links", {}).get("checkout"):
            ...
            return {
                "api_url": mollie_checkout_url,
                "extra_params": urls.url_parse(mollie_checkout_url).decode_query(),
                "qr_src": qr_src,
            }
        else:
            return {"api_url": payment_data.get("redirectUrl"), "ref": self.reference}
    ...
    def _send_refund_request(self, amount_to_refund=None, create_refund_transaction=True):
        ...
        refund_tx = super()._send_refund_request(amount_to_refund=amount_to_refund, create_refund_transaction=True)
        payment_data = self.provider_id._api_mollie_get_payment_data(self.provider_reference, force_payment=True)
        refund_data = self.provider_id._api_mollie_refund(
            amount_to_refund, self.currency_id.name, payment_data.get("id")
        )
    ...
    def _create_payment(self, **extra_create_values):
        ...
        if self.provider_id._get_code() == "mollie":
            ...
            if mollie_method and mollie_method.journal_id:
                ...
                payment_method_line = mollie_method.journal_id.inbound_payment_method_line_ids.filtered(
                    lambda l: l.code == mollie_method_payment_code
                )
            ...
            if mollie_method.code == "voucher":
                ...
                if remainder_method_code:  # if there is remainder amount
                    ...
                    remainder_payment_method_line = (
                        remainder_method.journal_id.inbound_payment_method_line_ids.filtered(
                            lambda l: l.code == reminder_mollie_method_payment_code
                        )
                    )
                    ...
                    if primary_journal != remainder_journal:
                        voucher_amount = sum(
                            [float(voucher["amount"]["value"]) for voucher in mollie_payment["details"]["vouchers"]]
                        )
                        ...
                        remainder_create_values = {
                            **extra_create_values,
                            "amount": float(
                                mollie_payment["details"]["remainderAmount"]["value"]
        ...
        if self.invoice_ids and self.mollie_reminder_payment_id:
            (self.invoice_ids.line_ids + self.mollie_reminder_payment_id.line_ids).filtered(
                lambda line: line.account_id == self.mollie_reminder_payment_id.destination_account_id
    ...
    def _get_received_message(self):
        ...
        if message and self.state == "done" and self.mollie_reminder_payment_id:
            message += _(
                "\nThe payment remaining amount is posted: %s", self.mollie_reminder_payment_id._get_html_link()
            )
        return message
    ...
    def _create_mollie_order_or_payment(self):
        ...
        if (
            (not method_record.mollie_enable_qr_payment)
            and "sale_order_ids" in self._fields
            and len(self.sale_order_ids) == 1
            and float_compare(self.sale_order_ids.amount_total, self.amount, precision_digits=2) == 0
        ):
            # Order API
    ...
    def _mollie_create_payment_record(self, api_type, silent_errors=False):
        ...
        result = self.provider_id._api_mollie_create_payment_record(
            api_type, payment_data, params=params, silent_errors=silent_errors
        )
    ...
    def _mollie_prepare_payment_payload(self, api_type):
        ...
        params = {}
        payment_data = {
            "method": const.PAYMENT_METHODS_MAPPING.get(self.payment_method_code, self.payment_method_code),
            "amount": {"currency": self.currency_id.name, "value": "%.2f" % self.amount},
            "metadata": {
        ...
        if api_type == "order":
            ...
            method_record = self.provider_id.payment_method_ids.filtered(
                lambda m: m.code == self.payment_method_id.code
            )
    ...
    def _mollie_get_order_lines(self, order):
        ...
        for line in order.order_line.filtered(lambda l: not l.display_type):  # ignore notes and section lines
            line_data = {
                ...
                "unitPrice": {"currency": line.currency_id.name, "value": "%.2f" % line.price_reduce_taxinc},
                "totalAmount": {
            ...
            if line.product_id.type == "service":
                line_data["type"] = (
                    "digital"  # We are considering service product as digital as we don't do shipping for it.
                )
        ...
        if self.payment_method_code == "billie":
            if not partner.commercial_company_name:
                raise ValidationError(
                    _("Company name is necessary for Billie payments. Go to address and add company name.")
                )
    ...
    def _process_refund_transactions_status(self):
        ...
        for transection in refund_transactions:
            if transection.provider_reference:
                ...
                refund_data = transection.provider_id._api_mollie_refund_data(
                    source_reference, transection.provider_reference
                )
                if refund_data and refund_data.get("id"):
                    if refund_data.get("status") == "refunded":
                        ...
                        self._set_canceled(
                            "Mollie: " + _("Mollie: failed due to status: %s", refund_data.get("status"))
                        )
